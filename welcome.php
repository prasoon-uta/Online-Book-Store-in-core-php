...
$stmt = $dbh->prepare('SELECT * FROM ShoppingBasket where `username`="'.$_SESSION['username'].'"');
$stmt->execute();
while ($row = $stmt->fetch()) {
 $_SESSION['basketID']=$row['basketId'];   
 }

//Fetch basket data for the customer
 $stmt = $dbh->prepare(' SELECT * FROM `contains` WHERE `basketID` = "'.$_SESSION['basketID'].'"');
 $stmt->execute();
 $count=0;
while ($row = $stmt->fetch()) {
    
	//Query to fetch book details like price,title and then store it into basket session
	$stmtBook=$dbh->prepare("SELECT * FROM `book` WHERE `ISBN` LIKE '".$row['ISBN']."'");
    $stmtBook->execute();	
	while ($rowInner = $stmtBook->fetch()) {
	$titleTemp =$rowInner['title']	;
	$priceTemp = $rowInner['price'];
	}
	
	
	$item_array=array(
	'item_isbn' => $row['ISBN'],
	'item_quantity' => $row['number'],
	'item_title' => $titleTemp,
	'item_price' => $priceTemp
	);
...
//$stmt = $dbh->prepare("SELECT * FROM `book` WHERE `ISBN`= (SELECT `ISBN` from author WHERE `name`="."'".$_POST['search']."')");
if(isset($_POST['btnAuthor']))
 $stmt = $dbh->prepare("SELECT `author`.`name`, `book`.* FROM `author` LEFT JOIN `writtenby` ON `author`.`SSN` = `writtenby`.`SSN` LEFT JOIN `book` ON `writtenby`.`ISBN` = `book`.`ISBN` WHERE (( `name` ='".$_POST['search']."'))");
 ...
 //$stmtStock = $dbh->prepare("SELECT SUM(`number`) FROM `stocks` WHERE `ISBN` LIKE '777'");
